...
import { getEffectiveResetTime, parseTimeString, getCurrentPeriodDate } from '$lib/utils';
import {
  getDomainConfigs,
  getDomainConfig,
  getGlobalSettings,
 ...
 */
export async function handleResetAlarm(
  domain: string,
  onReevaluate: () => void
): Promise<void> {
  ...
  if (tracking?.startedAt) {
    ...
    if (config && elapsed > 0) {
      ...
      console.log(`[TimeWarden] Accumulated ${elapsed.toFixed(1)}s to old period for ${domain}`);
    }
  }
...
export async function checkMissedResets(): Promise<void> {
  ...
  for (const config of storage.domains) {
    ...
    console.log(`[TimeWarden] Checked reset for ${config.domain} (current period: ${currentDate})`);
  }
}
