...
import type { DayOfWeek, NotificationRule, StatusResponse } from '$lib/types';
import { formatBadgeText, getCurrentPeriodDate, getEffectiveLimit } from '$lib/utils';
import { getGraceRemainingSeconds, isDomainInGracePeriod, startGracePeriod } from './block-manager';
...
export function scheduleReevaluation(): void {
    reevaluateQueue = reevaluateQueue
        .then(() => reevaluateAllDomainsImpl())
        .catch((err) => {
...
async function startTracking(domain: string, reason: 'focused' | 'audible'): Promise<void> {
    ...
    await scheduleTrackingAlarms(
        domain,
        usage.timeSpentSeconds,
        usage.limitSeconds,
        usage.notifications,
    );
...
export async function handleNotificationAlarm(alarmName: string): Promise<void> {
    ...
    const rules =
        config.useGlobalNotifications === false && config.notificationRules
...
export async function getStatusForDomain(domain: string): Promise<StatusResponse> {
    ...
    const limitSeconds =
        usage?.limitSeconds ?? getEffectiveLimit(config, String(new Date().getDay()) as DayOfWeek);
...
export async function togglePause(domain: string): Promise<{
    ...
    if (pausedDomains.has(domain)) {
        ...
        console.log(
            `[TimeWarden] Resumed ${domain} (paused ${pauseElapsed.toFixed(1)}s, ${remaining.toFixed(0)}s allowance left)`,
        );
        ...
        console.log(
            `[TimeWarden] Paused ${domain} (${remainingAllowance.toFixed(0)}s allowance remaining)`,
        );
...
export function getPauseState(
    domain: string,
    pausedSecondsFromStorage: number,
    allowanceSeconds: number,
): {
    isPaused: boolean;
    pauseRemainingSeconds: number;
...
export async function updateBadge(): Promise<void> {
    ...
    if (status.isInGracePeriod) {
        const graceText =
            status.graceRemainingSeconds > 0 ? String(status.graceRemainingSeconds) : '0';
    ...
    const remainingPercent =
        status.limitSeconds > 0 ? (status.timeRemainingSeconds / status.limitSeconds) * 100 : 100;
