  ...
  for (const [domain, entry] of map) {
    const avgSession =
      entry.allSessionDurations.length > 0
        ? entry.allSessionDurations.reduce((a, b) => a + b, 0) / entry.allSessionDurations.length
        : 0;
...
export function fillDateGaps(summaries: DaySummary[], range: '7d' | '14d' | '30d'): DaySummary[] {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    result.push(
      dateMap.get(dateStr) ?? { date: dateStr, totalSeconds: 0, domains: [] }
    );
  }
...
export function computeSessionAnalytics(
  ...
  for (const day of usage) {
    const domainUsages = domain
      ? day.domains.filter((d) => d.domain === domain)
      : day.domains;
