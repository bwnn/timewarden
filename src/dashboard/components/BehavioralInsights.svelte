...
<section>
  <h2 class="text-lg font-semibold text-gray-900 mb-4">Behavioral Insights</h2>

  <div class="grid gap-4 md:grid-cols-2">
    <!-- Key insights -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <h3 class="text-sm font-medium text-gray-500 mb-3">Key Insights</h3>
      <div class="space-y-3">
        {#if data.mostUsedDomain}
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-gray-900">Most Used</div>
        ...
        {#if data.mostCompulsiveDomain}
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-gray-900">Most Compulsive</div>
              <div class="text-sm text-gray-500">
        ...
        {#if peakHours.length > 0}
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-gray-900">Peak Hours</div>
              <div class="text-sm text-gray-500">
        ...
        {#if data.totalTimeSavedSeconds > 0}
          <div class="flex items-start gap-3">
            ...
            <div>
              <div class="text-sm font-medium text-gray-900">Time Saved</div>
              <div class="text-sm text-gray-500">
    ...
    <!-- Weekday vs Weekend -->
    <div class="bg-white rounded-lg border border-gray-200 p-4">
      <h3 class="text-sm font-medium text-gray-500 mb-3">Weekday vs Weekend</h3>
      <div class="space-y-4">
        <div>
          <div class="flex items-center justify-between text-sm mb-1">
            <span class="text-gray-600">Weekdays (avg/day)</span>
            <span class="font-medium text-gray-900">{formatTimeRemaining(data.weekdayAvgSeconds)}</span>
          </div>
        ...
        <div>
          <div class="flex items-center justify-between text-sm mb-1">
            <span class="text-gray-600">Weekends (avg/day)</span>
            <span class="font-medium text-gray-900">{formatTimeRemaining(data.weekendAvgSeconds)}</span>
          </div>
          <div class="w-full h-3 bg-gray-100 rounded-full">
            <div
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-4 md:col-span-2">
      <h3 class="text-sm font-medium text-gray-500 mb-3">Usage by Hour of Day</h3>
      <div class="flex items-end gap-0.5 h-20">
        {#each data.usageByHour as seconds, hour}
          <div class="flex-1 flex flex-col items-center gap-0.5">
            <div
              class="w-full rounded-t transition-all"
              style="height: {seconds > 0 ? Math.max(4, (seconds / maxHourUsage) * 100) : 0}%; background-color: {seconds > 0 ? `rgba(139, 92, 246, ${0.3 + 0.7 * (seconds / maxHourUsage)})` : '#f3f4f6'}"
            ></div>
            {#if hour % 3 === 0}
              <span class="text-xs text-gray-400" style="font-size: 9px">{formatHourShort(hour)}</span>
            {/if}
          </div>
        {/each}
      </div>
