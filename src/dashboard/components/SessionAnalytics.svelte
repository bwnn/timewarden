    ...
    <div class="grid gap-4 md:grid-cols-2">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-4">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <div class="text-2xl font-bold text-gray-900">{data.totalSessions}</div>
            <div class="text-xs text-gray-500">Total sessions</div>
          </div>
          <div>
            <div class="text-2xl font-bold text-gray-900">{formatTimeRemaining(data.longestSessionSeconds)}</div>
            <div class="text-xs text-gray-500">Longest session</div>
          </div>
          <div>
            <div class="text-2xl font-bold text-gray-900">{formatTimeRemaining(data.averageSessionSeconds)}</div>
            <div class="text-xs text-gray-500">Avg session length</div>
          </div>
          <div>
            ...
            <div class="text-xs text-gray-500">Avg time between</div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-4">
        <h3 class="text-sm font-medium text-gray-500 mb-3">Sessions by Day</h3>
        <div class="flex items-end gap-1.5 h-24">
          {#each data.sessionsByDay as count, i}
            <div class="flex-1 flex flex-col items-center gap-1">
              <div class="w-full flex flex-col items-center justify-end h-16">
                {#if count > 0}
                  <span class="text-xs text-gray-500 mb-0.5">{count}</span>
                {/if}
                <div
              ...
              <span class="text-xs text-gray-400">{dayNames[i]}</span>
            </div>
          {/each}
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-4 md:col-span-2">
        <h3 class="text-sm font-medium text-gray-500 mb-3">Sessions by Hour of Day</h3>
        <div class="flex items-end gap-0.5 h-20">
          {#each data.sessionsByHour as count, hour}
            <div class="flex-1 flex flex-col items-center gap-0.5">
              <div
                class="w-full rounded-t transition-all"
                style="height: {count > 0 ? Math.max(4, (count / maxHourSessions) * 100) : 0}%; background-color: {count > 0 ? `rgba(59, 130, 246, ${0.3 + 0.7 * (count / maxHourSessions)})` : '#f3f4f6'}"
              ></div>
              {#if hour % 3 === 0}
