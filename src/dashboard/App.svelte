<script lang="ts">
  ...
  import { getDashboardData } from '$lib/messaging';
  ...
  async function loadData() {
    try {
      ...
      allUsage = data.usage;
      // domains and settings available in data if needed
    } catch (err) {
  ...
  <div class="bg-white border-b border-gray-200">
    <div class="max-w-5xl mx-auto px-6 py-4">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-xl font-semibold text-gray-900">TimeWarden Dashboard</h1>
          <p class="text-sm text-gray-500 mt-0.5">Analytics and usage insights</p>
        </div>
        <div class="flex items-center gap-3">
          <DateRangeSelector value={range} onchange={handleRangeChange} />
          <button
            type="button"
            class="text-sm text-gray-600 hover:text-gray-900 px-3 py-1.5 rounded-lg border border-gray-200 hover:bg-gray-50 transition-colors"
    ...
    {:else}
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="text-sm text-gray-500">Total Time Tracked</div>
          <div class="text-2xl font-bold text-gray-900 mt-1">{formatTotalTime(totalTimeSeconds)}</div>
          <div class="text-xs text-gray-400 mt-0.5">in {range === '7d' ? '7' : range === '14d' ? '14' : '30'} days</div>
        </div>
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="text-sm text-gray-500">Most Used Domain</div>
          <div class="text-2xl font-bold text-gray-900 mt-1">{mostUsedDomain?.domain ?? '--'}</div>
          {#if mostUsedDomain}
            <div class="text-xs text-gray-400 mt-0.5">{formatTotalTime(mostUsedDomain.totalSeconds)}</div>
          {/if}
        </div>
        <div class="bg-white rounded-lg border border-gray-200 p-4">
          <div class="text-sm text-gray-500">Times Blocked</div>
          <div class="text-2xl font-bold {domainsAtLimit > 0 ? 'text-red-600' : 'text-emerald-600'} mt-1">{domainsAtLimit}</div>
          <div class="text-xs text-gray-400 mt-0.5">domain-day blocks</div>
        </div>
      </div>
      ...
      <!-- Footer -->
      <div class="mt-12 pb-8 text-center text-xs text-gray-400">
